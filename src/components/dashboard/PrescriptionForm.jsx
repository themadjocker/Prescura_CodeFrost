...
export function PrescriptionForm({ onSuccess }) {
  ...
  useEffect(() => {
    if (transcript) {
      setFormData((prev) =>
        !prev.problem ? { ...prev, problem: transcript } : prev
      );
    }
  }, [transcript]);
  ...
  const handleAIGenerate = async () => {
    ...
    setTimeout(() => {
      const problems = getCommonProblems();
      ...
      if (match) {
        ...
        setProtocols(match.protocols.map((p) => ({ ...p })));
      } else {
        setFormData((prev) => ({ ...prev, urgency: "Low" }));
        setProtocols([
          {
            name: "General Wellness Tabs",
            freq: "1-0-0",
            duration: "5 days",
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center">
              ...
              <Badge
                variant={
                  formData.urgency === "Critical" || formData.urgency === "High"
                    ? "critical"
                    : "default"
                }
              >
            ...
            <div className="space-y-3">
              {protocols.map((p, idx) => (
                <div
                  key={idx}
                  className="bg-white/60 p-3 rounded-xl border border-white shadow-sm flex flex-col md:flex-row gap-3 items-start md:items-center"
                >
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-12 gap-3 w-full">
                    <div className="md:col-span-4">
                      <input
                        ...
                        value={p.name}
                        onChange={(e) =>
                          handleProtocolChange(idx, "name", e.target.value)
                        }
                    ...
                    <div className="md:col-span-2">
                      <input
                        ...
                        value={p.freq}
                        onChange={(e) =>
                          handleProtocolChange(idx, "freq", e.target.value)
                        }
                    ...
                    <div className="md:col-span-2">
                      <input
                        ...
                        value={p.duration}
                        onChange={(e) =>
                          handleProtocolChange(idx, "duration", e.target.value)
                        }
                        className="w-full bg-transparent border-b border-slate-200 p-1 text-sm focus:outline-none focus:border-primary-500"
                      />
                    </div>
                    <div className="md:col-span-4">
                      <input
                        ...
                        value={p.notes}
                        onChange={(e) =>
                          handleProtocolChange(idx, "notes", e.target.value)
                        }

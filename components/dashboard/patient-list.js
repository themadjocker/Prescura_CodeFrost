            ...
            <CardContent className="p-0">
                {loading ? (
                    ...
                    <div className="overflow-x-auto">
                                ...
                                {patients.map((p) => (
                                    <tr key={p.id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="p-4 align-top">
                                            ...
                                            {p.age && <div className="text-xs text-slate-500 mt-1">Age: {p.age}</div>}
                                        </td>
                                        <td className="p-4 align-top">
                                            <div className="text-slate-700 font-medium">{p.problem || 'N/A'}</div>
                                        </td>
                                        <td className="p-4 align-top">
                                            <div className="space-y-2">
                                                {p.protocols && Array.isArray(p.protocols) ? (
                                                    p.protocols.map((m, i) => (
                                                        <div key={i} className="text-sm bg-white border border-slate-100 p-2 rounded-lg shadow-sm">
                                                            <div className="font-bold text-slate-700">{m.name}</div>
                                                            <div className="text-xs text-slate-500 flex gap-2 mt-1">
                                                                <span className="bg-slate-100 px-1.5 rounded">{m.freq}</span>
                                                                <span className="bg-slate-100 px-1.5 rounded">{m.duration}</span>
                                        ...
                                        <td className="p-4 align-top">
                                            <span className={`px-2 py-1 rounded text-xs font-bold uppercase border ${getUrgencyColor(p.urgency)}`}>
                                                {p.urgency}
                                            </span>
                                        </td>
                                    </tr>
                        ...
                        {patients.length === 0 && (
                            <div className="p-8 text-center text-slate-400">No patient records found.</div>
                        )}
                    </div>
                )}
            </CardContent>

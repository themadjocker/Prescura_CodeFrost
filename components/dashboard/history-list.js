function HistoryList() {
    ...
    // Initial Load - Get actual patients
    React.useEffect(() => {
        const fetchRealPatients = async () => {
             const { data } = await window.supabase.from('patients').select();
             if(data) {
                 // Mark them as "Admitted" for the feed
                 const formatted = data.map(d => ({ ...d, status: 'Admitted' }));
                 setHistory(formatted.slice(0, 5)); // Show last 5
             }
        };
        fetchRealPatients();
    }, []);

    // Mock Realtime Feed - Simulates "Viewers" or "Incoming Triage"
    React.useEffect(() => {
        const commonProblems = window.supabase.getCommonProblems();
        const names = ["Alex", "Jordan", "Taylor", "Morgan", "Casey", "Riley", "Jamie", "Quinn", "Avery", "Parker"];
        ...
        const interval = setInterval(() => {
            ...
            setHistory(prev => [newEvent, ...prev].slice(0, 8)); // Keep list short
            ...
            // Remove highlight
            setTimeout(() => {
                setHistory(prev => prev.map(h => h.id === newEvent.id ? { ...h, isNew: false } : h));
            }, 2000);
                        ...
                        >
                            <div className="flex justify-between items-start">
                                <div className="flex items-center gap-3">
                                    <div className={`w-2 h-2 rounded-full ${item.urgency === 'High' || item.urgency === 'Critical' ? 'bg-red-500 animate-pulse' : 'bg-slate-300'}`}></div>
                                    <div>
                                        <div className="font-bold text-slate-800 text-sm">{item.patient}</div>
                                        <div className="text-xs text-slate-500">{item.problem || 'Check-in'}</div>
                                    </div>
                                </div>
                                <div className={`text-[10px] font-bold px-2 py-1 rounded border ${getStatusColor(item.urgency)}`}>
                                    {item.urgency?.toUpperCase() || 'NORMAL'}
                                </div>
                            </div>
                        </div>
                    ))}
                    {history.length === 0 && <div className="text-center text-slate-400 text-sm py-4">Waiting for incoming signals...</div>}
                </div>
            </CardContent>
        </Card>
    );

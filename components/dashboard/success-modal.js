function SuccessModal({ isOpen, onClose, data }) {
    const qrRef = React.useRef(null);

    React.useEffect(() => {
        if (isOpen && data && qrRef.current) {
            qrRef.current.innerHTML = '';
            try {
                // Simplify data for QR code to avoid too much text
                const qrData = {
                    p: data.patient,
                    d: data.problem,
    ...
    const medsList = data.protocols ? data.protocols.map(p => p.name).join(', ') : 'Check Details';
    const shareUrl = `https://wa.me/?text=${encodeURIComponent(`Rx for ${data.patient}: ${medsList}`)}`;
            ...
            <div className="relative glass-panel w-full max-w-md p-0 animate-[slideUp_0.4s_ease-out] shadow-2xl overflow-hidden">
                ...
                <div className="p-8 flex flex-col items-center gap-6 bg-white/80">
                    ...
                    <div className="text-center space-y-1">
                        ...
                        <p className="text-sm text-slate-500">ID: {Math.random().toString(36).substring(7).toUpperCase()}</p>
                    </div>
